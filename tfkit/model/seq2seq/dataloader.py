...
def get_feature_from_data(tokenizer, maxlen, input, previous, target=None, ntarget=None, reserved_len=0,
    ...
    for t_input in t_input_list:  # -2 for cls and sep
        ...
        row_dict['previous'] = [0] * maxlen
        tokenized_previous_id = []
        target_mask_id = [1]

        if len(previous) > 0:
            tokenized_previous_id = tokenizer.convert_tokens_to_ids(previous)
            target_mask_id = [1] * (len(tokenized_previous_id)) + [1]
            target_start = len(tokenized_previous_id)
        ...
        if target is not None:
            ...
            target_end = len(t_target_id)
            t_target_id.extend([-1] * (maxlen - len(t_target_id)))
        ...
        tokenized_previous_id.extend([0] * (maxlen - len(tokenized_previous_id)))
