...
def get_multiclas_data_from_file(fpath):
    ...
    with open(fpath, "r") as infile:
        ...
        headers = ["input"] + [
            "target_" + str(i) for i in range(len(fieldnames) - 1)
        ]
                    ...
                    if tok.UNIVERSAL_SEP in item:
                        for i in item.split(tok.UNIVERSAL_SEP):
                            task_label_dict[task].append(
                                i) if i not in task_label_dict[
                                    task] else task_label_dict[task]
                    else:
                        task_label_dict[task].append(
                            item) if item not in task_label_dict[
                                task] else task_label_dict[task]
        ...
        for rows in nlp2.read_csv_chunk(fpath, ","):
            ...
            for row in rows:
                ...
                for pos, item in enumerate(row[start_pos:]):
                    ...
                    task = headers[0] + "_" + headers[pos] + is_multi_label
                    item = item.strip()
                    targets = (item.split(tok.UNIVERSAL_SEP)
                               if tok.UNIVERSAL_SEP in item else [item])
                    targets = [
                        task_label_dict[task][task_label_dict[task].index(
...
def get_tag_data_from_file(fpath,
                           text_index: int = 0,
                           label_index: int = 1,
                           separator=" "):
    task_label_dict = defaultdict(list)
...
def get_tag_data_from_file_col(fpath,
                               text_index: int = 0,
                               label_index: int = 1,
                               separator=" ",
                               **kwargs):
    ...
    with open(fpath, "r", encoding="utf-8") as f:
        ...
        for line in tqdm(lines):
            ...
            if len(rows) > 1:
                if rows[label_index] not in labels and len(
                        rows[label_index]) > 0:
