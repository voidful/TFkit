                ...
                for pos, item in enumerate(row[start_pos:]):
                    ...
                    if tok.UNIVERSAL_SEP in item:
                        for i in item.split(tok.UNIVERSAL_SEP):
                            task_label_dict[task].append(i) if i not in task_label_dict[
                                task
                            ] else task_label_dict[task]
                    else:
                        task_label_dict[task].append(
                            item
                        ) if item not in task_label_dict[task] else task_label_dict[
                            task
                        ]
        ...
        for rows in nlp2.read_csv_chunk(fpath, ","):
            ...
            for row in rows:
                ...
                for pos, item in enumerate(row[start_pos:]):
                    ...
                    item = item.strip()
                    targets = (
                        item.split(tok.UNIVERSAL_SEP)
                        if tok.UNIVERSAL_SEP in item
                        else [item]
                    )
                    targets = [
                        task_label_dict[task][task_label_dict[task].index(target)]
...
def get_tag_data_from_file(
    fpath, text_index: int = 0, label_index: int = 1, separator=" "
):
    task_label_dict = defaultdict(list)
    task = "tag"
    labels = []
    ...
    for rows in nlp2.read_csv_chunk(fpath, ","):
        ...
        for row in rows:
            chunk.append(
                {
                    "task": task,
                    "input": row[text_index].strip(),
                    "target": row[label_index].strip(),
                    "separator": separator,
                }
            )
        yield chunk
...
def get_tag_data_from_file_col(
    fpath, text_index: int = 0, label_index: int = 1, separator=" ", **kwargs
):
