...
class BaseTextGeneratePredictor(BasePredictor):
    ...
    def wrap_input(
        ...
        return {
            "filtersim":
            json.loads(str(filtersim).lower()),
            "topK":
            int(topK),
            "no_repeat":
            json.loads(str(no_repeat).lower()),
            "eos_num":
            int(eos_num),
            "topP":
            float(topP),
            ...
            int(reserved_len),
            "mode":
            mode[0] if isinstance(mode, list) else mode.lower(),
            "merge_strategy":
            merge_strategy[0]
            if isinstance(merge_strategy, list) else merge_strategy.lower(),
            "handle_exceed":
            handle_exceed,
            "task":
            task,
...
class ClassificationPredictor(BasePredictor):
    ...
    def wrap_input(
        ...
        return {
            ...
            mode[0] if isinstance(mode, list) else mode.lower(),
            "task":
            task if len(task) > 0 else list(self.model.tasks_detail.keys())[0],
            "input":
            input,
            "reserved_len":
            int(reserved_len),
    ...
    def processing(self, input_args):
        with torch.no_grad():
            ...
            for items in proc.preprocess({
                    ...
                    "input": input_args["input"]
            }):
                feature = proc.postprocess(
                    {
                        **items,
                        **{
                            "task_dict": self.model.tasks
                        }
                    },
                    self.model.tokenizer,
                    self.model.maxlen,
            ...
            if input_args["topK"] < 2:
                ret_result.append([
                    i[input_args["task"]] for i in predictions["max_item"]
                    if input_args["task"] in i
                ][0])
                ret_detail.append(predictions)
            else:
                task_map = [
                    i[input_args["task"]] for i in predictions["label_prob"]
                    if input_args["task"] in i
                ][0]
                ret_result.append(
                    sorted(task_map, key=task_map.get,
            ...
            for detail in ret_detail:
                ...
                results_entropy.append(
                    Categorical(probs=torch.tensor(
                        result_value)).entropy().data.tolist())
    ...
    def processing(self, input_args):
        with torch.no_grad():
            ...
            for items in proc.preprocess({
                ...
                feature = proc.postprocess(items, self.model.tokenizer,
                                           self.model.maxlen)
            ...
            start_dict = [
                i["start"] for i in result["label_prob_all"] if "start" in i
            ][0]
            end_dict = [
                i["end"] for i in result["label_prob_all"] if "end" in i
            ][0]
            ...
            sorted_start = sorted(start_dict.items(),
                                  ...
                                  reverse=True)[:50]
            sorted_end = sorted(end_dict.items(),
                                key=lambda item: item[1],
                                reverse=True)[:50]
            ...
            answer_results = sorted(answers,
                                    key=lambda answers: answers[2],
                                    reverse=True)[:input_args["topK"]]
            ret_result.append([
                "".join(
                    self.model.tokenizer.convert_tokens_to_string(
                        raw_input[ans[0]:ans[1]])) for ans in answer_results
            ])
            ...
            for detail in non_empty_detail:
                ...
                prob_sum = [
                    sum(x) for x in zip(
                        list(detail["label_prob_all"][0]["start"].values()),
            ...
            for items in proc.preprocess({
                ...
                for token_pred, token_map in zip(result["label_prob_all"],
                    ...
                    predicted_pos_prob[token_map["pos"]]["prob"].append(
                        max_prob)
                predicted_pos_prob[token_map["pos"]]["entropy"].append(
                    max_entropy)
            ...
            for mapping in ret_result:
                for k, y in mapping.items():
                    if input_args["start_contain"] in y:
                        ...
                        after_start = False
            if len(target_str[0]
                   ) > input_args["minlen"] and target_str not in output:
                output.append(target_str)
            output = [[
                ner,
                tag.replace(input_args["start_contain"],
                            "").replace(input_args["end_contain"], ""),
...
class AutoRegressivePredictor(BaseTextGeneratePredictor):
    ...
    def processing(self, input_args):
        ...
        if tok.UNIVERSAL_SEP in input_args["input"]:
            previous = self.model.tokenizer.tokenize(input_args["input"].split(
                tok.UNIVERSAL_SEP)[-1])
        ...
        with torch.no_grad():
            while True:
                ...
                for seq in sequences:
                    if (sep_tok not in seq[0]
                            or seq[0].count(sep_tok) < input_args["eos_num"]):
                        tokens, score = seq
                        if not tokens:
                            tokens = previous
                        ...
                        for items in proc.preprocess({
                            ...
                            feature_dict = proc.postprocess_batch(
                                proc.postprocess(items, self.model.tokenizer,
                                                 self.model.maxlen))

                        # check input exceed
                        if (len(tokens) >= self.model.maxlen or
                            ("prev" in feature_dict and
                             len(feature_dict["prev"]) >= self.model.maxlen) or
                            ("input" in feature_dict and
                             len(feature_dict["input"]) >= self.model.maxlen)):
                            exceed = True
                        ...
                        label_prob = (predictions["label_prob"]
                                      if "label_prob" in predictions else
                        ...
                        if "top" in input_args["mode"]:
                            ...
                            if "topk" in input_args["mode"]:
                                ...
                                decode_range = max(input_args["decodenum"],
                                                   input_args["topK"])
                                prob_norm = [
                                    float(i) / sum(sample_list)
                                ...
                                index_overP = [
                                    i for i, x in enumerate(topP_list)
                                    if x > input_args["topP"]
                                ]
                                index_overP = (0 if len(index_overP) < 1 else
                                               index_overP[0])
                        ...
                        # greedy / beam search
                            ...
                            for k, v in label_prob[:50]:
                                if (input_args["no_repeat"] and len(tokens) > 0
                                        and tokens[-1] == k) or len(k) < 1:
                                    continue
                                candidate = [
                                    tokens + [k],
                ...
                for i in sequences:
                    ...
                    if ((i[0].count(sep_tok) >= input_args["eos_num"])
                            or len(i[0]) > self.model.maxlen
                            or 0 < input_args["decode_maxlen"] < len(i[0])):
                        stop += 1
