...
class BaseTextGeneratePredictor(BasePredictor):
    def wrap_input(
        ...
        return {
            ...
            "merge_strategy": merge_strategy[0]
            if isinstance(merge_strategy, list)
            else merge_strategy.lower(),
    ...
    def processing(self, input_args):
        with torch.no_grad():
            ...
            for items in proc.preprocess(
                {"task": input_args["task"], "input": input_args["input"]}
            ):
                feature = proc.postprocess(
                    {**items, **{"task_dict": self.model.tasks}},
                    self.model.tokenizer,
                    self.model.maxlen,
                )
            ...
            if input_args["topK"] < 2:
                ...
                task_map = [
                    i[input_args["task"]]
                    for i in predictions["label_prob"]
                    if input_args["task"] in i
                ][0]
                ret_result.append(
                    sorted(task_map, key=task_map.get, reverse=True)[
                        : input_args["topK"]
                    ]
                )
                ret_detail.append(predictions)
        ...
        if input_args["merge_strategy"] == "count":
            ...
            for detail in ret_detail:
                ...
                results_entropy.append(
                    Categorical(probs=torch.tensor(result_value))
                    .entropy()
                    .data.tolist()
                )
...
class QuestionAnsweringPredictor(BasePredictor):
    ...
    def processing(self, input_args):
        with torch.no_grad():
            ...
            start_dict = [i["start"] for i in result["label_prob_all"] if "start" in i][
                0
            ]
            ...
            answers = []
            sorted_start = sorted(
                start_dict.items(), key=lambda item: item[1], reverse=True
            )[:50]
            sorted_end = sorted(
                end_dict.items(), key=lambda item: item[1], reverse=True
            )[:50]
            ...
            answer_results = sorted(
                answers, key=lambda answers: answers[2], reverse=True
            )[: input_args["topK"]]
            ret_result.append(
                [
                    "".join(
                        self.model.tokenizer.convert_tokens_to_string(
                            raw_input[ans[0] : ans[1]]
                        )
                    )
            ...
            for detail in non_empty_detail:
                prob_start = detail["label_prob_all"][0]["start"][
                    int(detail["label_map"][0]["start"])
                ]
                prob_end = detail["label_prob_all"][0]["end"][
                    int(detail["label_map"][0]["end"])
                ]
                prob_sum = [
                    sum(x)
                    for x in zip(
                ...
                results_entropy.append(
                    Categorical(probs=torch.tensor(prob_sum)).entropy().data.tolist()
                )
    ...
    def processing(self, input_args):
        with torch.no_grad():
            ...
            for items in proc.preprocess(
                ...
                feature = proc.postprocess(
                    {**items, **{"task_dict": self.model.labels}},
                    self.model.tokenizer,
                    self.model.maxlen,
                )
                for k, v in feature.items():
                ...
                result = self.model.forward(feature, eval=True)
                for token_pred, token_map in zip(
                    result["label_prob_all"], result["token_word_mapping"]
                ):
                    token_prob = list(token_pred.values())[0]
                    max_label = max(token_prob, key=token_prob.get)
                    max_prob = token_prob[max_label]
                    max_entropy = (
                        Categorical(probs=torch.tensor(list(token_prob.values())))
                        .entropy()
                        .data.tolist()
                    )
            ...
            output = [
                [
                    ner,
                    tag.replace(input_args["start_contain"], "").replace(
                        input_args["end_contain"], ""
                    ),
                ]
            ...
            while True:
                ...
                for seq in sequences:
                    if (
                        ...
                        # check input exceed
                        if (
                            len(tokens) >= self.model.maxlen
                            or (
                                "prev" in feature_dict
                                and len(feature_dict["prev"]) >= self.model.maxlen
                            )
                            or (
                                "input" in feature_dict
                                and len(feature_dict["input"]) >= self.model.maxlen
                            )
                        ):
                            exceed = True
                        ...
                        predictions = self.model.forward(
                            ...
                            beamsearch=input_args["decodenum"] > 1,
                            max_return=max(input_args["decodenum"], input_args["topK"]),
                        )
                        label_prob = (
                            predictions["label_prob"]
                            if "label_prob" in predictions
                            else [predictions["max_item"]]
                        )
                        # topK topP
                        if "top" in input_args["mode"]:
                            ...
                            if "topk" in input_args["mode"]:
                                ...
                                prob_norm = [
                                    float(i) / sum(sample_list) for i in sample_list
                                ]
                                ...
                                index_overP = [
                                    i
                                    for i, x in enumerate(topP_list)
                                    if x > input_args["topP"]
                                ]
                                index_overP = (
                                    0 if len(index_overP) < 1 else index_overP[0]
                                )
                ...
                for i in sequences:
                    ...
                    if (
                        (i[0].count(sep_tok) >= input_args["eos_num"])
