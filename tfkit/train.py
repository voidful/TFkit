...
def parse_train_args(args):
    ...
    exceed_mode = nlp2.function_get_all_arg_with_value(tok.handle_exceed)[
        'mode']
    parser.add_argument("--batch", type=int, default=20,
                        help="batch size, default 20")
    parser.add_argument("--lr", type=float, nargs='+',
                        default=[5e-5], help="learning rate, default 5e-5")
    ...
    parser.add_argument("--savedir", type=str, default="checkpoints/",
                        help="task saving dir, default /checkpoints")
    ...
    parser.add_argument("--train", type=str, nargs='+',
                        required=True, help="train dataset path")
    parser.add_argument("--test", type=str, nargs='+',
                        required=True, help="test dataset path")
...
def load_model_and_datas(tokenizer, pretrained, accelerator, model_arg, input_arg):
    ...
    for model_class_name, train_file, test_file in zip_longest(input_arg.get('task'), input_arg.get('train'),
        ...
        train_ds = get_dataset(train_file, model_class,
                               tokenizer, ds_parameter)
        ...
        # append to max len
        train_ds_maxlen = train_ds.__len__() if train_ds.__len__(
        ) > train_ds_maxlen else train_ds_maxlen
        test_ds_maxlen = test_ds.__len__() if test_ds.__len__(
        ) > test_ds_maxlen else test_ds_maxlen
...
def main(arg=None):
    input_arg, model_arg = parse_train_args(
        sys.argv[1:]) if arg is None else parse_train_args(arg)
    accelerator = Accelerator()
    ...
    pretrained = load_pretrained_model(
        input_arg.get('config'), input_arg.get('task'))
    ...
    logger.write_log("=======================")
    [logger.write_log(str(key) + " : " + str(value))
    ...
    for epoch in range(start_epoch, start_epoch + input_arg.get('epoch')):
        ...
        save_model(models, input_arg, models_tag, epoch, fname,
                   logger, add_tokens=add_tokens, accelerator=accelerator)
