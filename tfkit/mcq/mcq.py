...
class MCQ(nn.Module):
    ...
    def forward(self, batch_data, eval=False):
        ...
        prediction_output = self.model(sequence_output)
        prediction_scores = self.batched_index_select(prediction_output.squeeze(-1), 1, indices_tensor)
        if eval:
            self.eval()
            result_dict = {
                'label_map': [],
            ...
            masked_lm_loss = loss_fct(prediction_scores, loss_tensors)
