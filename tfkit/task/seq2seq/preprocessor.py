...
class Preprocessor(GeneralNLPPreprocessor):
    ...
    def preprocess_component_convert_to_id(self,
        ...
        tokenized_input, tokenized_target, n_target, b_target = (
            item["input"],
        ...
        if tokenized_target is None:
            ...
            if "neg" in likelihood or "both" in likelihood:
                ...
                if n_target is None:
                    ...
                    ntext_arr = [
                        ntext.strip() for ntext in n_target.split(
                            tok.tok_sep(self.tokenizer))
                    ]
                else:
                ...
                for neg_text in ntext_arr:
                    yield {
                        ...
                        "previous":
                        self.tokenizer.convert_tokens_to_ids(previous),
                        "target":
                        self.tokenizer.convert_tokens_to_ids(tokenized_target),
                        "ntarget":
                        self.tokenizer.encode(neg_text),
                    }
            else:
                yield {
                    "input":
