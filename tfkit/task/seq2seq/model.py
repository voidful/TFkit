    ...
    def forward(self, batch_data, eval=False, beamsearch=False, max_return=1, **kwargs):
        ...
        if eval:
            ...
            result_dict["max_item"] = (
                self.tokenizer.convert_ids_to_tokens(max_item_id),
                max_item_prob,
            )
            if max_return > 1:
                topK = torch.topk(softmax_score, max_return)
                prob_result = [
                    (self.tokenizer.convert_ids_to_tokens(tid), prob)
                    for prob, tid in zip(
                        topK.values.data.tolist(), topK.indices.data.tolist()
                    )
                ]
            ...
            if self.selfkd:
                ...
                for decoder_hidden in prediction_all_hidden[:-1]:
                    ...
                    lm_loss += selfkdloss_fct(
                        student.view(-1, self.vocab_size),
            ...
            if "btarget" in batch_data:
                ...
                if not torch.all(backtran_tensors.eq(-1)).item():
                    ...
                    backtran_hidden = (
                        backtran_predation["encoder_last_hidden_state"]
                        if "encoder_last_hidden_state" in backtran_predation
                        else prediction["last_hidden_state"]
                    )
                    backtran_loss = F.cosine_similarity(
                        self.encoder_hidden, backtran_hidden
                    ).mean()
