...
from tfkit.utility.model import load_predict_parameter
from tfkit.utility.model import load_trained_model
...
def parse_eval_args(args):
    ...
    parser.add_argument(
        "--config", type=str, help="pre-trained task path after add token"
    )
    parser.add_argument(
        "--metric",
        required=True,
        type=str,
        choices=["emf1", "nlg", "clas", "er"],
        help="evaluate metric",
    )
    parser.add_argument(
        "--valid", required=True, type=str, nargs="+", help="evaluate data path"
    )
    parser.add_argument(
        "--tag", type=str, help="evaluate task tag for select multi-task task"
    )
    parser.add_argument(
        "--print", action="store_true", help="print each pair of evaluate data"
    )
    parser.add_argument(
...
def main(arg=None):
    with torch.no_grad():
        eval_arg, model_arg = (
            parse_eval_args(sys.argv[1:]) if arg is None else parse_eval_args(arg)
        )
        ...
        for model_path in models:
            ...
            if (
                "decodenum" in predict_parameter
                and int(predict_parameter["decodenum"]) > 1
            ):
                eval_metrics = [
                    EvalMetric(model.tokenizer)
                    for _ in range(int(predict_parameter["decodenum"]))
                ]
            else:
            ...
            for chunk in tqdm(get_data_item):
                for i in chunk:
                    ...
                    for eval_pos, eval_metric in enumerate(eval_metrics):
                        ...
                        if "qa" in model_type:
                            processed_target = " ".join(
                                input.split(" ")[int(target[0]) : int(target[1])]
                            )
                            if len(result) > 0:
                                predicted = (
                                    result[0][0]
                            ...
                            "onebyone" in model_type
                            or "seq2seq" in model_type
                            or "clm" in model_type
                        ):
                            processed_target = target
                            if len(result) < eval_pos:
                                print(
                                    "Decode size smaller than decode num:",
                            ...
                            predicted = " ".join(
                                [
                                    list(d.values())[0]
                                    for d in result_dict[0]["label_map"]
                                ]
                            )
            ...
            for eval_pos, eval_metric in enumerate(eval_metrics):
                ...
                if "mode" in predict_parameter:
                    para_mode = (
                        predict_parameter["mode"][0]
                        if isinstance(predict_parameter["mode"], list)
                        else predict_parameter["mode"].lower()
                    )
                ...
                with open(
                    outfile_name + "_each_data_score.csv", "w", encoding="utf8"
                ) as edsf:
                    eds = csv.writer(edsf)
