...
def main(arg=None):
    with torch.no_grad():
        eval_arg, model_arg = (parse_eval_args(sys.argv[1:])
                               if arg is None else parse_eval_args(arg))
        ...
        for model_path in models:
            ...
            predict_parameter = load_predict_parameter(model, model_arg,
                                                       eval_arg.get("panel"))

            if ("decodenum" in predict_parameter
            ...
            for chunk in tqdm(get_data_item):
                for i in chunk:
                    ...
                    for eval_pos, eval_metric in enumerate(eval_metrics):
                        ...
                        eval_metric.add_record(input, predicted,
                                               processed_target,
                                               eval_arg.get("metric"))
            ...
            for eval_pos, eval_metric in enumerate(eval_metrics):
                ...
                if "mode" in predict_parameter:
                    para_mode = (predict_parameter["mode"][0] if isinstance(
                        predict_parameter["mode"], list) else
                                 predict_parameter["mode"].lower())
                    argtype += "_mode_" + str(para_mode)
                if "filtersim" in predict_parameter:
                    argtype += "_filtersim_" + str(
                        predict_parameter["filtersim"])
                outfile_name = model_path + argtype

                with open(outfile_name + "_predicted.csv",
                          "w",
                          encoding="utf8") as f:
                    writer = csv.writer(f)
                ...
                with open(outfile_name + "_each_data_score.csv",
                          "w",
                          encoding="utf8") as edsf:
                    eds = csv.writer(edsf)
                    with open(outfile_name + "_score.csv",
                              "w",
                              encoding="utf8") as f:
                        for i in eval_metric.cal_score(eval_arg.get("metric")):
                            f.write("TASK: " + str(i[0]) + " , " +
                                    str(eval_pos) + "\n")
                            f.write(str(i[1]) + "\n")
                            eds.writerows(i[2])
