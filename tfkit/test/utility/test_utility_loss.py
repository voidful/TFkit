...
class TestLoss(unittest.TestCase):
    outputs = Variable(
        torch.Tensor([[0.00000000000009, 5, 0.5], [0.00000000000000000001, 69, 9]]),
        requires_grad=False,
    )
    ...
    def testLabelSmoothingCrossEntropy(self):
        outputs = torch.Tensor(
            [[0.00000000000009, 5, 0.5], [0.00000000000000000001, 69, 9]]
        )
        ...
        self.assertTrue(
            criterion(outputs, onen_targets).item()
    ...
    def testDiceLoss(self):
        ...
        self.assertTrue(
            0.99 < custom_criterion(self.outputs, self.alln_targets).item() <= 1
        )
        self.assertTrue(
    ...
    def testNegativeCElLoss(self):
        ...
        self.assertTrue(
            criterion(outputs, targets).item()
            < custom_criterion(outputs, self.targets).item()
        )
        self.assertTrue(
            criterion(outputs, onen_targets).item()
            < custom_criterion(outputs, onen_targets).item()
    ...
    def testFocalLoss(self):
        ...
        self.assertTrue(
            criterion(self.outputs, self.targets)
        ...
        self.assertTrue(
            criterion(self.outputs, self.onen_targets)
