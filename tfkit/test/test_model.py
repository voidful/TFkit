...
class TestDataLoader(unittest.TestCase):
    ...
    def testClassifier(self):
        ...
        for k, v in feature.items():
            feature[k] = [v, v]
        model = tfkit.classifier.BertMtClassifier(tasks_detail={"taskA": ["a", "b"]},
                                                  model_config="voidful/albert_chinese_tiny")
        print(model(feature))
    ...
    def testQA(self):
        ...
        for k, v in feature.items():
            feature[k] = [v, v]
        model = tfkit.qa.BertQA(model_config="voidful/albert_chinese_tiny")
        print(model(feature))
        self.assertTrue(isinstance(model(feature), Tensor))
        print(model(feature, eval=True))
        model_dict = model(feature, eval=True)
    ...
    def testTag(self):
        ...
        for k, v in feature.items():
            feature[k] = [v, v]
        model = tfkit.tag.BertTagger(labels=["O", "A"],
                                     model_config="voidful/albert_chinese_tiny")
    ...
    def testOnce(self):
        ...
        for k, v in feature.items():
            feature[k] = [v, v]
        model = tfkit.gen_once.BertOnce(model_config="voidful/albert_chinese_tiny")
    ...
    def testOnebyone(self):
        ...
        tokenizer = BertTokenizer.from_pretrained('voidful/albert_chinese_tiny')
        feature = tfkit.gen_onebyone.get_feature_from_data(tokenizer, input=input, previous=previous, target=target,
                                                           maxlen=512)
        for k, v in feature.items():
            feature[k] = [v, v]
        model = tfkit.gen_onebyone.BertOneByOne(model_config="voidful/albert_chinese_tiny")
