...
class TestModel(unittest.TestCase):
    ...
    def testClas(self):
        ...
        maxlen = 512
        model = tfkit.task.clas.Model(tokenizer, pretrained, tasks_detail={"taskA": ["a", "b"]})
        preprocessor = tfkit.task.clas.Preprocessor(tokenizer, maxlen=maxlen, handle_exceed='start_slice',
                                                    reserved_len=0)
        for preprocessed_item in preprocessor.preprocess(
                {'task': 'taskA', 'input': input, 'target': target, 'task_dict': {"taskA": ["a", "b"]}}):
            feature = preprocessor.postprocess(preprocessed_item, tokenizer, maxlen=maxlen)
    ...
    def testGenerationModel(self):
        ...
        # tfkit.task.seq2seq, tfkit.task.once, tfkit.task.oncectc, tfkit.task.clm, tfkit.task.onebyone, tfkit.task.seq2seqbt
        for gmodel in [tfkit.task.seq2seq, tfkit.task.once, tfkit.task.oncectc, tfkit.task.clm, tfkit.task.onebyone,
                       tfkit.task.seq2seqbt]:
            print(str(gmodel))
            model = gmodel.Model(tokenizer, pretrained, maxlen=maxlen)
            preprocessor = gmodel.Preprocessor(tokenizer, maxlen=maxlen, handle_exceed='start_slice', reserved_len=0)
            for preprocessed_item in preprocessor.preprocess(
                    {'task': 'taskA', 'input': input}):
                print("preprocessed_item", preprocessed_item)
                feature = preprocessor.postprocess(preprocessed_item, tokenizer, maxlen=maxlen)
                for k, v in feature.items():  # simulate batch
                    feature[k] = [v, v]
            ...
            # TopK
            result, detail = model.predict(input=input, decodenum=3, mode='topK', topK=3, filtersim=False)
            print("topK", result)
            ...
            # beamsearch
            result, detail = model.predict(input=input, decodenum=3)
            print("beamsaerch", len(result), result, model_dict)
            self.assertTrue(len(result) == 3)
            self.assertTrue(isinstance(result, list))
            self.assertTrue(isinstance(detail, dict))

            # TopP
            result, detail = model.predict(input=input, decodenum=3, mode='topP', topP=0.8)
            ...
            print("test exceed 512", len(result), result, model_dict)
            self.assertTrue(isinstance(result, list))
            self.assertTrue(isinstance(detail, dict))
    ...
    def testQA(self):
        ...
        tokenizer = BertTokenizer.from_pretrained('voidful/albert_chinese_tiny')
        pretrained = AutoModel.from_pretrained('voidful/albert_chinese_tiny')
        model = tfkit.task.qa.Model(tokenizer, pretrained, maxlen=512)

        proc = tfkit.task.qa.Preprocessor(tokenizer, maxlen=512, handle_exceed='start_slice',
                                                    reserved_len=0)
        for items in proc.preprocess({"input": input}):
        ...
        # # test eval top k = 2
        # top_k_label, top_k_dict = task.predict(input=input, topK=2)
        # print("top_k_label", top_k_label)
        # self.assertTrue(len(top_k_label) == 2)

        # test exceed 512
        for merge_strategy in ['entropy', 'count', 'prob']:
            result, model_dict = model.predict(input=" ".join([str(i) for i in range(550)]),handle_exceed='start_slice',
    ...
    def testTag(self):
        ...
        model = tfkit.task.tag.Model(tokenizer=tokenizer, pretrained=pretrained, tasks_detail={"default": label})

        # test exceed 512
        for merge_strategy in ['count']:
